...
describe('plugin tests', function (this: any) {
  ...
  before(() => {
    ...
    console.log(
      '### serverless version: %s ###',
      new Serverless({ commands: ['print'], options: {}, serviceDir: null })
        .version,
    );
  });
  ...
  beforeEach(async () => {
    ...
    let compileHooks: any[] =
      serverless.pluginManager.hooks['package:setupProviderConfiguration'];
    compileHooks = compileHooks.concat(
      serverless.pluginManager.hooks['package:setupProviderConfiguration'],
      serverless.pluginManager.hooks['package:compileFunctions'],
      serverless.pluginManager.hooks['package:compileEvents'],
    );
  ...
  describe('defaultInherit not set', () => {
    ...
    describe('#constructor()', () => {
      ...
      it('should NOT initialize the plugin for non AWS providers', () => {
        assert.throws(
          () => new Plugin({ service: { provider: { name: 'not-aws' } } }),
        );
      });
    ...
    describe('#validateStatements', () => {
      ...
      it('should throw an error for invalid statement', () => {
        ...
        assert.throws(() => {
          plugin.validateStatements(badStatement);
        });
      });
    ...
    describe('#createRolesPerFunction', () => {
      it('should create role per function', () => {
        ...
        const helloInheritRole =
          compiledResources.HelloInheritIamRoleLambdaExecution;
        ...
        const streamHandlerRole =
          compiledResources.StreamHandlerIamRoleLambdaExecution;
        assertFunctionRoleName(
          'streamHandler',
        ...
        assert.isObject(
          policyStatements.find((s) => s.Action[0] === 'sns:Publish'),
          'sns dlq statements included',
        );
        const streamMapping =
          compiledResources.StreamHandlerEventSourceMappingDynamodbTest;
        assert.equal(
        ...
        const sqsHandlerRole =
          compiledResources.SqsHandlerIamRoleLambdaExecution;
        assertFunctionRoleName(
          'sqsHandler',
        ...
        const sqsMapping =
          compiledResources.SqsHandlerEventSourceMappingSQSMyQueue;
        ...
        const helloNoPerFunctionResource =
          compiledResources.HelloNoPerFunctionLambdaFunction;
        ...
        const helloEmptyIamStatementsRole =
          compiledResources.HelloEmptyIamStatementsIamRoleLambdaExecution;
        assertFunctionRoleName(
        ...
        const helloEmptyFunctionResource =
          compiledResources.HelloEmptyIamStatementsLambdaFunction;
        assert.isTrue(
      ...
      it('should do nothing when no functions defined', () => {
        ...
        for (const key in compiledResources) {
          if (
            ...
            Object.prototype.hasOwnProperty.call(compiledResources, key)
          ) {
    ...
    describe('#createRolesPerFunction', () => {
      ...
      it('should add permission policy arn when there is iamPermissionsBoundary defined', () => {
        const compiledResources =
          serverless.service.provider.compiledCloudFormationTemplate.Resources;
        plugin.createRolesPerFunction();
        const helloPermissionsBoundaryIamRole =
          compiledResources.HelloPermissionsBoundaryIamRoleLambdaExecution;
        const policyName =
          helloPermissionsBoundaryIamRole.Properties.PermissionsBoundary[
            'Fn::Sub'
          ];
        assert.equal(
          policyName,
          'arn:aws:iam::xxxxx:policy/your_permissions_boundary_policy',
        );
      });
      ...
      it('should add permission policy arn when there is iamGlobalPermissionsBoundary defined', () => {
        ...
        plugin.createRolesPerFunction();
        const defaultIamRoleLambdaExecution =
          compiledResources.IamRoleLambdaExecution;
        const policyName =
          defaultIamRoleLambdaExecution.Properties.PermissionsBoundary[
            'Fn::Sub'
          ];
        assert.equal(
          policyName,
          'arn:aws:iam::xxxxx:policy/permissions_boundary',
        );
      });
    });
  });
